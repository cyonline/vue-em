...
Vue.component("input-file",{
    template:
    '<div><label :for="name"><div class="el-input el-input-group el-input--small el-input-group--append">' +
    '<input type="text" placeholder="请选择文件" autocomplete="off" v-model="fileName" class="el-input__inner" readonly>' +
    '<div class="el-input-group__append">浏览</div></div></label>' +
    '<input style="display: none" type="file" :id="name" @change="updateValue($event.target.files)"/>'+
    '</div>',
    props:{
        name:{
            type:String,
    ...
    methods:{
        updateValue:function (value) {
            var val= value[0];
            if(value.length>0){
                this.fileName = val.name?val.name:"";
                this.fileData = value;
            }else{
                this.clearValue();
            }
...
Vue.component("file-upload", {
    template: '<div><label :for="name"><div class="el-input el-input-group el-input--small el-input-group--append">' +
        '<input type="text" placeholder="请选择文件" autocomplete="off" v-model="fileName" class="el-input__inner" readonly>' +
        '<div class="el-input-group__append">浏览</div></div></label>' +
        '<input style="display: none" type="file" :id="name" :accept="accept" @change="updateValue($event.target.files)"/>' +
        '</div>',
    props: {
        name: {
            type: String,
    ...
    methods: {
        updateValue: function(value) {
            console.log('----------------------')
            var accepts = this.accept.replace(/\*|\*,/g, '').split(',');
            var acceptsStr = accepts.join(',');
            // Object.entries(this.alias).forEach(([k, v]) => {
            //     acceptsStr = acceptsStr.replace(k, v);
            // });
            for (var key in this.alias) {
                acceptsStr = acceptsStr.replace(key, this.alias[key]);
            }
            var arr = [];
            if (value.length > 0) {
                arr = [value[0]];
            }
            var flag = arr.every(function(file) {
                var item = false;
                accepts.map(function(str) {
                    if (str == file.type) {
                        item = true;
                    }
                })
                if (!item && this.accept !== '*') {
                    this.$message.warning('只允许' + acceptsStr + '文件格式');
                    this.clearValue();
                    return false;
                }
                return true;
...
function isPosition (el) {
    var position = getStyle(el, 'position');
    console.log('position:', el, position)
    return position.match(/(relative|absolute|fixed)/);
};
...
function getPositionParent (el) {
    ...
    while (parent) {
        ...
        if (isPosition(parent)) {
            clientInfo.clientLeft += parent.clientLeft;
            clientInfo.clientTop += parent.clientTop;
        }
