...
Vue.component('project-tree', {
    template: '<div>' +
        '        <div v-if="filterable"><el-input v-model.trim="treeFilterText" :maxlength="50" placeholder="输入关键字搜索" size="small" icon="search"></el-input></div>' +
        '        <div>' +
        '           <el-tree ref="projectTree" :data="treeData" :current-node-key="currentNodeId" :show-checkbox="showCheckbox" :node-key="nodeKey" :props="props" :highlight-current="!showCheckbox"' +
        '           :default-expand-all="false" :default-expanded-keys="defaultExpandedKeys" :expand-on-click-node="true" ' +
        '           @node-click="clickPartTree" :filter-node-method="funFilterTree"></el-tree>' +
        '        </div>' +
        '      </div>',
    props: {
        type: {
...
Vue.component('tree-select', {
    template: '<el-select :popper-class="popperClass" :disabled="disabledSelect" size="small" v-model="curLabel" :placeholder="placeholder" class="vertical-middle" ' +
...
Vue.component('tree-select-corp', {
    template: '<tree-select ref="treeSelectCorp" :popper-class="popperClass" :cur-key="currentNodeId" :data="treeData" :props="treeProps" :node-key="nodeKey" :placeholder="placeholder" :expand-all="expandAll"' +
        ' :expanded="defaultExpandedKeys" :change="handleNodeChange" :disabled-select="disabledSelect" :disabled="setNodeDisabled"  :on-all="onAll"  :filterable="filterable" :clearable="clearable" >',
    props: {
...
var MultipleSelectCorp = Vue.extend({
    template: '<div class="projectTree_alert_box alert-box">' +
        ...
        '           <el-tree ref="projectTree" class="border_none" :data="treeData" show-checkbox :node-key="nodeKey" :default-checked-keys="defaultCheckedKeys" :default-expanded-keys="defaultExpandedKeys" :props="treeProps" @check-change="handleProjectTreeCheckChange"></el-tree>' +
        '       </div>' +
...
Vue.mixin({
    directives: {
        "format": {
            bind: function(el, binding, vnode) {
                ...
                if (type === "c") {
                    //监听数据更新为会计格式数据，手动触发（从后台返回数据时使用）
                    context.$watch("updateNumToThousand", function() {
                        let val = _this.value;
                        if (typeof val === "string" && val.indexOf(",") > -1) {
                            return;
                        }

                        let newVal = tool.moneyNum(val, place, defaultValue);
                    ...
                    switch (type) {
                        ...
                        case "fnc": //自定义格式
                            if (typeof option === "function") {
                                ...
                                //使用自定义格式函数，假如返回的是undefined，那么执行默认操作
                                let val = option.call(context, value, tool, opt, this);
                                if (val !== undefined) {
                                    value = val;
                                    break;
                                }
    ...
    data: function() {
        return {
            updateNumToThousand: false
        }
    },
    methods: {
        $parseNumToThousands: function() {
            this.updateNumToThousand = !this.updateNumToThousand;
        },
        $parseThousandToNum: function(val, defaultValue) {
            if (val === null || val === undefined || val === "") {
                return defaultValue || "";
            }

            return tool.parseNum(val);
        }
    }
});