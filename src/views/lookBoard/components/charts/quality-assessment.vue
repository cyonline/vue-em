...
export default {
  ...
  methods: {
    computedPosition:function(length) {
      ...
      console.log(length <= 15 ? this.start = 0 : this.start = (100 - Math.floor(15 / length * 100)),'111')
      if(length>=15){
    ...
    axisLabel: function(){
      if(this.chartData.xAxis.data.length >= 5){
        return 10
      }else {
        return 0
      }
    ...
    fetchData: function () {
      ...
      this.$$get('/api/reportBoardSpecialAssess/AssessChartData', subData)
        .then(function (res) {
